...
Nsweeps = 1500;       %number of sweeps over the lattice to 
Xmax=20;            %number of sites in x-direction
Ymax=20;            %number of sites in the y-direction
T=0:0.05:5;               %non-dimensional temperature value
interfaceEnergy=1;  %non-dimensional boundary energy

plotMat=false;      %plot simulation - turn off inside MATLAB Grader
randseed = randi(1000);       %used to ensure identical answers in stochastic simulations
rng(553);      %set the random seed
...
for q = 1:length(T)
    ...
    %     list = randi(Nsites,1,Nsites);%pick sites randomly
        ...
        for i=1:length(list)
            ...
            mat(ix,iy)=GlauberDynamicsMetropolis(mat,ix,iy,interfaceEnergy,T(q),Xmax,Ymax,Nspins);
    ...
    Mtotal = Mtotal(Nsweeps-100:Nsweeps);
    M(q) = sum(Mtotal,'all')/length(Mtotal);
end
toc
%% plot section
M(1) = 1;
